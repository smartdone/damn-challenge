...
contract UnstoppableVault is IERC3156FlashLender, ReentrancyGuard, Owned, ERC4626 {
    ...
    function getTotalSupplyTest() public view returns (uint256) {
        return convertToShares(totalSupply);
    }
    ...
    function flashLoan(
        ...
        if (amount == 0) revert InvalidAmount(0); // 如果amount为0，就revert
        if (address(asset) != _token) revert UnsupportedCurrency(); // enforce ERC3156 requirement
        uint256 balanceBefore = totalAssets();
        
        if (convertToShares(totalSupply) != balanceBefore) revert InvalidBalance(); // enforce ERC4626 requirement 会检测totalSupply和balanceBefore是否相同，只需要向池子里面转token，就会导致这个校验不通过
